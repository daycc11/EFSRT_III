...
<body class="bg-background-light dark:bg-background-dark font-display flex items-center justify-center min-h-screen p-4 bg-cover bg-center"
      style="background-image: linear-gradient(rgba(0,0,0,0.3), rgba(0,0,0,0.4)), url('<%=request.getContextPath()%>/img/fondo.png');">

<div class="w-full max-w-md mx-auto bg-white/95 dark:bg-gray-900/90 backdrop-blur-sm rounded-xl shadow-lg p-8 text-center">
    ...
    <h1 class="text-3xl font-bold text-gray-800 dark:text-white mt-4">Bienvenido Doctor</h1>
    <p class="text-gray-600 dark:text-gray-400 mt-2">Inicia sesión para continuar</p>

    <% if (errorLogin != null) { %>
        <p class="text-red-600 font-semibold mt-4 flex items-center justify-center gap-2">
            <span class="material-symbols-outlined text-red-600">error</span>
            <%= errorLogin %>
        </p>
    <% } %>

    <form class="space-y-6 mt-6" action="<%= request.getContextPath() %>/vistas/logeo/validarDoc.jsp" method="post">
        <div class="relative">
            <span class="material-symbols-outlined absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 dark:text-gray-500">badge</span>
        ...
        <div class="relative">
            <span class="material-symbols-outlined absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 dark:text-gray-500">lock</span>
