...
try {
	...
	ps = con.prepareStatement("SELECT * FROM Administracion WHERE dni=? AND contrasenia=?");
	ps.setString(1, dni);
	...
	if (rs.next()) {
		...
		response.sendRedirect("vistas/administracion/administracion.jsp");
	} else {
		request.setAttribute("errorLogin", "DNI o Contraseña incorrectos.");
		request.getRequestDispatcher("vistas/administracion/loginAdm.jsp").forward(request, response);
	}
} catch (Exception e) {
